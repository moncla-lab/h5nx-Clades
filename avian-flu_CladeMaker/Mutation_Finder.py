...
Input_Tree_Path = "flu_avian_h5n1_ha.JSON"
...
output_file_path = "unique_clades.txt"
...
def leaf_clades(Tree):
    ...
    for item in Tree.Objects: #for each item
        if item.branchType == "leaf": #if the item is a leaf
            ...
            if clade not in cladesAll: #making sure the clade does not already appear in our complete list of clades -- we only want each clade to show up once
...
def clade_heirarchy(all_clades, parentClades):
    ...
    for clade in all_clades: #for each clade in our list of all clades
        if len(clade) == 1 or "non" in clade or "like" in clade: #if our clade is 1-8-9-like, any of the non_GsGD clades, or is just a single number then that clade is equivalent to its parent clade
...
def leaf_nuc_muts(hierarchical_list, treeJSON):
    ...
    for k in treeJSON.Objects: #here we're going to go through every leaf and do the following code for each leaf we find
        if k.branchType == "leaf": #find all leaves
            ...
            if mutation_clade not in mutations.keys(): #here we need to check if the clade is already in the dict, if isn't we can just set the mutations to the clade
                mutations[mutation_clade] = [nucleotide_muts]
            
            else: #if the clade is already present, we don't want to overwrite the mutations we already have, so we're going to append it
...
def node_clades(treeJSON, leaf_clades):
    node_clade = {} #initializing the variable, just like usualy
        ...
        for mutation in HA[key]: #for each mutation in HA mutations
            ...
            for clade in HA.keys(): #for clade in our keys
                if clade != key: # as long as our clade is not the key we are appending to
                    if mutation in HA[clade]: #check if our mutation is present
...
"""
Prints our unique HA and Nuc mutations in a pretty way, if wanted
"""
def pretty_print(alluniqueHA, alluniqueNuc, Nodes, output_file_path):

    lines_for_file1 = []
    lines_for_file = []
    lines_for_file2 = []
    lines_for_file.append("Here are your unique HA mutations:")

    for key in alluniqueHA.keys():
        lines_for_file.append("Clade:")
        lines_for_file.append(key)
        lines_for_file.append("Mutations: ")
        lines_for_file.append(str(alluniqueHA[key]))

    lines_for_file1.append("Here are your unique Nucleotide mutations: ")

    for key in alluniqueNuc.keys():
        lines_for_file1.append("Clade:")
        lines_for_file1.append(key)
        lines_for_file1.append("Mutations: ")
        lines_for_file1.append(str(alluniqueNuc[key]))

    lines_for_file2.append("UNIQUE NODES")
    for key in Nodes.keys():
        lines_for_file2.append("Clade:")
        lines_for_file2.append(key)
        lines_for_file2.append("Node: ")
        lines_for_file2.append(Nodes[key])
    ...
    with open(output_file_path, "w") as output_file:
        ...
        output_file.write("*****************************************************************************")
        output_file.write('\n')
        output_file.write('\n'.join(lines_for_file1))
        output_file.write('\n')
        output_file.write('\n')
        output_file.write('\n')
        output_file.write("*****************************************************************************")
        output_file.write('\n')
        output_file.write('\n'.join(lines_for_file2))
